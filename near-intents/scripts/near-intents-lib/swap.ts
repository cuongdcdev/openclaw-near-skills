...
export async function intentSwap(
  ...
  let userAccountId = env.wallet.ACCOUNT_ID || '';
  let userPrivateKey = env.wallet.PRIVATE_KEY || '';
  
  const near = await env.set_near(userAccountId, userPrivateKey);

  console.log("swap.ts: user account id : " , userAccountId , " user private key: ", userPrivateKey, " tokenDataIn: " , tokenDataIn);
...
async function _intentSwap(
  ...
  const matchesIn = tokenData.filter((obj: any) => obj.symbol.toUpperCase() === tokenIn.toUpperCase());
  if (!matchesIn.length) {
    return false;
  }
  
  const matchesOut = tokenData.filter((obj: any) => obj.symbol.toUpperCase() === tokenOut.toUpperCase());
  ...
  console.log(`[_intentSwap] Current balance of ${tokenIn}: ` , balanceResult);
  ...
  throw new Error("result is not provided after maximum retries");
}
