...
export async function getWithdrawMessageToSign(
  ...
  const storageDeposit = availableBalance > FT_MINIMUM_STORAGE_BALANCE_LARGE
    ? "0"
    : FT_MINIMUM_STORAGE_BALANCE_LARGE.toString();

  console.log(`[getWithdrawMessageToSign] Generating withdraw message for ${amount} of ${token} to ${receiverId} on ${blockchain} | storage deposit: ${storageDeposit}`);
...
export async function getIntentsBalance(env: Environment, accountId: string, tokenData: any[]): Promise<Array<{ symbol: string, balance: string, intents_token_id: string } | []>> {
  try {
    ...
    console.log('[getIntentsBalance] Fetching balances for account:', accountId, 'with token IDs:', tokenIds);
    ...
    for (let i = 0; i < tokenIds.length; i++) {
      if (result[i] && result[i] !== '0') {
        ...
        if (token) {
          ...
          const formattedBalance = new Decimal(result[i])
    ...
    console.log('[getIntentsBalance] Intents balance output:', balanceOutput);
